...
export interface IRdfStoreIndexBTreeOptions {
  ...
  linearProbes?: number;
  /**
   * The fraction of a leaf that a batch fills new leaves up to, which leaves room for later inserts without a split.
   * Filled completely, nearly every leaf splits on the first insert after a batch.
   * On WatDiv, 0.9 made the first 50,000 inserts after a batch about 15% faster than 1.
   * @default 0.9
   */
  leafFill?: number;
}
...
export class RdfStoreIndexBTree implements IRdfStoreIndex<number, boolean> {
  ...
  /**
   * The number of quads that a batch fills a new leaf with.
   */
  private readonly leafFillCount: number;
  /**
  ...
  public constructor(options: IRdfStoreOptions<number>, indexOptions: IRdfStoreIndexBTreeOptions = {}) {
    ...
    this.leafFillCount = Math.max(1, Math.floor(this.leafCapacity * (indexOptions.leafFill ?? 0.9)));
