...
describe('TermDictionaryNumberTermTypeRecordFullTerms', () => {
  let dict: ITermDictionary<number>;

  beforeEach(() => {
    dict = new TermDictionaryNumberTermTypeRecordFullTerms();
  });
  ...
  describe('encode', () => {
    it('should encode named nodes', () => {
      ...
      expect(dict.encode(DF.namedNode('ex:s2'))).toEqual(8);
      expect(dict.encode(DF.namedNode('ex:s3'))).toEqual(16);
    ...
    it('should encode blank nodes', () => {
      expect(dict.encode(DF.blankNode('bs1'))).toEqual(1);
      expect(dict.encode(DF.blankNode('bs2'))).toEqual(9);
      expect(dict.encode(DF.blankNode('bs3'))).toEqual(17);
    ...
    it('should encode literals', () => {
      expect(dict.encode(DF.literal('abc'))).toEqual(3);
      expect(dict.encode(DF.literal('def'))).toEqual(11);
      expect(dict.encode(DF.literal('abc', DF.namedNode('ex:d')))).toEqual(19);
      expect(dict.encode(DF.literal('abc', 'en-us'))).toEqual(2);
    ...
    it('should not encode variables', () => {
      expect(() => dict.encode(DF.variable('v1')))
        .toThrow('Encoding of Variables is not supported');
    });

    it('should encode the default graph', () => {
      expect(dict.encode(DF.defaultGraph())).toEqual(4);
      expect(dict.encode(DF.defaultGraph())).toEqual(4);
    });

    it('should not encode quoted quads', () => {
      expect(() => dict.encode(DF.quad(
        DF.namedNode('s'),
        DF.namedNode('p'),
        DF.namedNode('o'),
        DF.namedNode('g1'),
      ))).toThrow('Encoding of Quads is not supported');
    });

    it('should encode mixed terms nodes', () => {
      expect(dict.encode(DF.namedNode('s'))).toEqual(0);
      expect(dict.encode(DF.blankNode('s'))).toEqual(1);
      expect(dict.encode(DF.literal('s'))).toEqual(3);
  ...
  describe('encodeOptional', () => {
    it('should return undefined for non-encoded terms', () => {
      ...
      expect(dict.encodeOptional(DF.namedNode('ex:s2'))).toEqual(8);
      expect(dict.encodeOptional(DF.namedNode('ex:s3'))).toEqual(16);
  ...
  describe('decode', () => {
    ...
    it('should decode encoded terms', () => {
      ...
      expect(dict.encode(DF.blankNode('s'))).toEqual(1);
      expect(dict.encode(DF.literal('s'))).toEqual(3);
      expect(dict.encode(DF.literal('s', 'en-us'))).toEqual(2);

      expect(dict.decode(0)).toEqualRdfTerm(DF.namedNode('s'));
      expect(dict.decode(1)).toEqualRdfTerm(DF.blankNode('s'));
      expect(dict.decode(3)).toEqualRdfTerm(DF.literal('s'));
      expect(dict.decode(2)).toEqualRdfTerm(DF.literal('s', 'en-us'));
    });
  });
  ...
  describe('encodings', () => {
    ...
    it('should return for a non-empty dictionary', () => {
      ...
      expect([ ...dict.encodings() ]).toEqual([ 0, 1, 3 ]);
    });
  });
