...
describe('TermDictionaryQuotedIndexed', () => {
  ...
  beforeEach(() => {
    dict = new TermDictionaryWrapperTermEncoded(
  ...
  describe('decode', () => {
    it('should not throw when entry does not exist', () => {
      expect(() => dict.decode(0)).not.toThrow();
    });

    it('should throw during usage of the term when entry does not exist', () => {
      const term = dict.decode(0);
      expect(() => term.value).toThrow('The value 0 is not present in this dictionary');
    });
