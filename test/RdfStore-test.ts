...
describe('RdfStore', () => {
  ...
  describe('matchBindings iterator', () => {
    ...
    it('should end an empty result even if its consumer only subscribes later', async() => {
      const iterator = store.matchBindings(
        BF,
        DF.namedNode('s-none'),
        DF.variable('p'),
      ...
      // An iterator that has already ended when it is handed out emits its `end` event on the task
      // scheduler, so a consumer that only subscribes after that never sees it and waits forever.
