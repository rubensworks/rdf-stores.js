...
import type { ITermsCardinalitySet } from '../../lib/index/ITermsCardinalitySet';
...
describe('TermsCardinalitySets', () => {
  ...
  describe('using a number dictionary', () => {
    beforeEach(() => {
    ...
    describe('that has one term', () => {
      ...
      describe('remove', () => {
        it('should be able to remove a term', () => {
          expect(index.remove(
            dictionary.encode(DF.namedNode('s')),
          )).toEqual(0);
        });
    ...
    describe('that has two terms', () => {
      ...
      describe('add', () => {
        it('should increase when adding the same term', () => {
          expect(index.add(dictionary.encode(DF.namedNode('s')))).toEqual(3);
        });
      });
      ...
      describe('getTerms', () => {
        it('should still have only 1 term', () => {
          expect(index.getTerms().length).toEqual(1);
        });
      });
    });
  });
